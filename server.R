...
FullResults <- do.call("rbind", lapply(paste0("www/Results_TopVar/", TopVariantsFiles), function(file){
  ResTrait <- fread(file, data.table = FALSE)
  ResTrait <- subset(ResTrait, PvalPRISM < (5*10^(-8)/69))
  if(nrow(ResTrait) > 0)
    return(cbind.data.frame(Trait1 = gsub("^(.*Results_)(.+)(_.*\\.csv)$", "\\2", file), ResTrait))
}))
RSIDs <- unique(FullResults[, c("rsid", "variant")])
FullResults <- FullResults[, -which(colnames(FullResults) == "rsid")]
PleioType <- by(FullResults$variant, FullResults$SynthPleio, list)
AllVariants <- unique(FullResults$variant)

VariantListPleio <- by(FullResults$variant, FullResults$SynthPleio, table)
VariantListPleio <- lapply(VariantListPleio, function(variants) {
...
TopVar <- unique(FullResults$variant[order(FullResults$PvalPRISM, decreasing = FALSE)[1:30000]])
VariantListPleio_reduced <- lapply(VariantListPleio, function(res) subset(res, SNPid %in% TopVar))
...
getVariantNetwork <- function(Vex){
    ...
    # Get variant results
    # Get variant results
    FullPleio <- subset(FullResults, variant == Vex)
...
nbTraits <- length(unique(c(TraitCharacteristics$phenotype_Y, TraitCharacteristics$phenotype_X)))
...
names(PleioColors) <- c("Direct Effect", "Vertical Pleiotropy", "Confounder Pleiotropy")
...
shinyServer(function(input, output, session){
    ...
    output$TraitCausal <- renderImage({
    	...
    	if(! file.exists("www/CircosPleiotropic relationships.jpeg")){
	    	...
	    	ALLCOUNT$Trait1Explicit <- TraitDef$description_short[ match(ALLCOUNT$TraitA, TraitDef$phenotype) ]
	    	ALLCOUNT$Trait2Explicit <- TraitDef$description_short[ match(ALLCOUNT$TraitB, TraitDef$phenotype) ]
	    	
	    	###Circos Plot
	    	
	    	colors <- list(
	    		"Confounder Pleiotropy" = "#a23c33", 
	    	...
	    	pourcirc <- ALLCOUNT[,c("Trait1Explicit", "Trait2Explicit", "Count")]
	    	...
	    	chordDiagram(
	    		pourcirc, #
	    		order = unique(TraitDef$description_short[order(TraitDef$Category)]),
	    		grid.col = grid.col,
	    		col = pourcirc$col_mat,
	    		directional = 1,
	    	...
	    	circos.track(track.index = 1, panel.fun = function(x, y) {
	    		...
	    		circos.text(mean(xlim), mean(ylim), sector.index, col = "white", cex = 0.5, facing = "clockwise", niceFacing = TRUE)
	    	}, bg.border = NA)
    		dev.off()
    	}
    ...
    output$VariantNetworkTabs <- renderUI({
        if(is.null(re_values$chroms_selected) | ! "variant" %in% names(input))
            return(NULL)
        if(input$variant == "")
            return(NULL)
